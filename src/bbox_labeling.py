...
def make_interface():
    interface = toloka.project.TemplateBuilderViewSpec(
        # Component that selects areas in images
        view=tb.ImageAnnotationFieldV1(
            tb.OutputData('result'),  # Path for writing output data
            tb.InputData('image'),  # Getter for the input image
            shapes={'rectangle': True},
            # At least one area should be selected
            validation=tb.RequiredConditionV1(hint='Please select an area')
        )
    )
    return interface
